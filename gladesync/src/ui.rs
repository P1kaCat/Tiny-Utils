...
const PIN_LABEL_T: i32 = 210;
const PIN_LABEL_B: i32 = 234;
const PIN_FIELD_X: i32 = 28;
const PIN_FIELD_Y: i32 = 236;
const PIN_FIELD_W: i32 = 364;
const PIN_FIELD_H: i32 = 40;
...
const SETUP_ACTION_BTN_T: i32 = 284;
const SETUP_ACTION_BTN_B: i32 = 330;
...
const SETUP_BACK_BTN_T: i32 = 338;
const SETUP_BACK_BTN_B: i32 = 374;
...
struct UIState {
    ...
    dialog_mode: u8,
    stop_countdown: i32,
    /// Unix timestamp (ms) of the last connect/host attempt; 0 = never.
    last_attempt_ms: u64,
}
...
pub fn start_ui_thread(network: Arc<NetworkManager>) {
    thread::spawn(move || unsafe {
        ...
        if !game_hwnd.is_null() {
            crate::hook::set_game_hwnd(game_hwnd);
        }
...
unsafe fn paint_dialog(hwnd: HWND, state: &UIState) {
    ...
    SetTextColor(hdc, TEXT_MUTED);
    let label_pseudo: Vec<u16> = "Nickname:\0".encode_utf16().collect();
    ...
    // ── "PIN:" label + field (shown in both HOST and JOIN setup modes) ──
    if state.dialog_mode == MODE_HOST || state.dialog_mode == MODE_JOIN {
        let label_pin: Vec<u16> = "PIN:\0".encode_utf16().collect();
        let mut r_lpin = RECT { left: 30, top: PIN_LABEL_T, right: 200, bottom: PIN_LABEL_B };
        DrawTextW(hdc, label_pin.as_ptr(), -1, &mut r_lpin, (DT_SINGLELINE | DT_VCENTER) as u32);
        draw_field_bg(hdc, PIN_FIELD_X, PIN_FIELD_Y, PIN_FIELD_W, PIN_FIELD_H);
    }
...
unsafe fn update_field_visibility(state: &mut UIState) {
    match state.dialog_mode {
        MODE_IDLE | MODE_HOST_ACTIVE | MODE_JOIN_ACTIVE => {
            ShowWindow(state.hwnd_ip, SW_HIDE);
            ShowWindow(state.hwnd_port, SW_HIDE);
            ShowWindow(state.hwnd_pin, SW_HIDE);
        }
        MODE_HOST => {
            ShowWindow(state.hwnd_ip, SW_HIDE);
            fit_edit_field(state.hwnd_port, state.edit_font, 108, SETUP_FIELD_Y, 204, SETUP_FIELD_H);
            ShowWindow(state.hwnd_port, SW_SHOW);
            fit_edit_field(state.hwnd_pin, state.edit_font, PIN_FIELD_X, PIN_FIELD_Y, PIN_FIELD_W, PIN_FIELD_H);
            ShowWindow(state.hwnd_pin, SW_SHOW);
        }
        MODE_JOIN => {
...
unsafe extern "system" fn wnd_proc(
    ...
    match msg {
        WM_CREATE => {
            ...
            let hwnd_pin = CreateWindowExW(
                0,
                edit_class.as_ptr(),
                std::ptr::null(),
                WS_CHILD | (ES_AUTOHSCROLL as u32) | WS_TABSTOP | (ES_NUMBER as u32),
                PIN_FIELD_X, PIN_FIELD_Y, PIN_FIELD_W, PIN_FIELD_H,
                hwnd,
                ID_EDIT_PIN as *mut c_void,
                inst,
                std::ptr::null(),
            );
            ...
            SendMessageW(hwnd_pin, WM_SETFONT, edit_font as usize, 1);
            SendMessageW(hwnd_pin, EM_SETLIMITTEXT, 4, 0); // max 4 digits

            let state = &mut *(state_ptr as *mut UIState);
            state.hwnd_pseudo = hwnd_pseudo;
            state.hwnd_ip = hwnd_ip;
            state.hwnd_port = hwnd_port;
            state.hwnd_pin = hwnd_pin;
            ...
            fit_edit_field(hwnd_pin, edit_font, PIN_FIELD_X, PIN_FIELD_Y, PIN_FIELD_W, PIN_FIELD_H);
            0
        }
                    ...
                    match state.dialog_mode {
                        ...
                        MODE_HOST => {
                            if x >= 30 && x <= 390 && y >= SETUP_ACTION_BTN_T && y <= SETUP_ACTION_BTN_B {
                                // ── Anti-spam: 5-second cooldown on host attempts ──
                                let now = now_ms();
                                if state.last_attempt_ms != 0 && now < state.last_attempt_ms + 5000 {
                                    state.status_text = format!("Please wait {}s before retrying.",
                                        ((state.last_attempt_ms + 5000 - now) / 1000) + 1);
                                    InvalidateRect(hwnd, std::ptr::null(), 1);
                                } else {
                                ...
                                let mut pin_buf = [0u16; 16];
                                GetWindowTextW(state.hwnd_pin, pin_buf.as_mut_ptr(), 16);
                                let pin = String::from_utf16_lossy(&pin_buf)
                                    .trim_matches('\0').trim().to_string();

                                match state.network.start_host(port, pin) {
                                    Ok(_) => {
                        ...
                        MODE_JOIN => {
                            if x >= 30 && x <= 390 && y >= SETUP_ACTION_BTN_T && y <= SETUP_ACTION_BTN_B {
                                // ── Anti-spam: 5-second cooldown on connect attempts ──
                                let now = now_ms();
                                if state.last_attempt_ms != 0 && now < state.last_attempt_ms + 5000 {
                                    state.status_text = format!("Please wait {}s before retrying.",
                                        ((state.last_attempt_ms + 5000 - now) / 1000) + 1);
                                ...
                                thread::spawn(move || unsafe {
                                    let res = net_clone.connect_to_host(&addr, pin);
